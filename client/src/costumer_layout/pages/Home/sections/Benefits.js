    ...
    <section className="relative flex flex-col justify-center items-center w-full padding-x background-gradient1 ">
      ...
      <div className="grid grid-cols-1 tablet:grid-cols-2 gap-6 mx-auto p-5 max-sm:p-8">
        {benefits.map((benefits, index) => (
          <div
            key={index}
            className="flex flex-row max-[370px]:flex-col max-[370px]:p-2 p-6 sm:rounded-full w-full rounded-[16px] shadow-lg bg-secondary_1 h-44 "
          >
            <div className="flex items-center justify-center gap-4 h-full">
           
              <FontAwesomeIcon
                icon={benefits.icon}
                className="rounded-full self-center bg-white p-5 w-12 h-12 max-sm:p-4 max-sm:w-4 max-sm:h-4 max-[350px]:w-2 max-[350px]:h-2 max-[350px]:p-2 "
              />
              
              </div>

              <div className="flex flex-col justify-start p-2 text-left gap-2 ml-4 ">
                <h1 className="max-[350px]:text-[12px] max-[350px]:leading-[14px] max-sm:text-[14px] max-sm:leading-[18px] text-[18px] leading-[22px] font-semibold text-white">
                  {benefits.title}
                </h1>
                <p className="max-sm:text-[10px] max-[350px]:text-[8px] text-xs text-white ">
